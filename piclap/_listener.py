...
class Listener():
    ...
    def __printInfo(self):
        print("Device Name\t\t:", self.device.defaultDevice['name'])
...
class Device:
    ...
    def __setInputDevice(self):
        ...
        for i in range(self.input.get_device_count()):
            dev = self.input.get_device_info_by_index(i).get('name')
            if("Teensy" in dev):
                devId = i
                break
            elif("default" in dev):
                ...
                break
        self.deviceId = devId
        self.defaultDevice = self.input.get_device_info_by_index(self.deviceId)
    ...
    def openStream(self):
        ...
        self.stream = self.input.open(format=pyaudio.paInt16,
                                      ...
                                      input_device_index=self.deviceId,
                                      input=True,
